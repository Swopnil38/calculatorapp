import 'package:flutter/material.dart';

// ignore: constant_identifier_names
enum Operator { Add, Subtract, Multiply, Divide }

List<String> buttonText = [
  'C',
  '*',
  '/',
  '<-',
  '1',
  '2',
  '3',
  '+',
  '4',
  '5',
  '6',
  '-',
  '7',
  '8',
  '9',
  '*',
  '%',
  '0',
  '.',
  '=',
];

class CalculatorView extends StatefulWidget {
  const CalculatorView({Key? key}) : super(key: key);

  @override
  State<CalculatorView> createState() => _CalculatorViewState();
}

class _CalculatorViewState extends State<CalculatorView> {
  final TextEditingController _textController = TextEditingController();

  int num1 = 0;
  int num2 = 0;
  Operator? operator;

  static const Map<String, Operator> operatorMap = {
    '+': Operator.Add,
    '-': Operator.Subtract,
    '*': Operator.Multiply,
    '/': Operator.Divide,
  };

  void _calculate(String value) {
    try {
      int selectedNumber = operator == null ? num1 : num2;
      selectedNumber = int.parse('$selectedNumber$value');
      operator == null ? num1 = selectedNumber : num2 = selectedNumber;
      _textController.text = selectedNumber.toString();
    } catch (e) {
      switch (value) {
        case 'C':
          _clear();
          break;
        case '+':
        case '-':
        case '*':
        case '/':
          _setOperator(value);
          break;
        case '=':
          _performCalculation();
          break;
      }
    }
  }

  void _clear() {
    _textController.text = '';
    num1 = 0;
    num2 = 0;
    operator = null;
  }

  void _setOperator(String value) {
    operator = operatorMap[value];
    _textController.text = operator == null ? '$num1$value' : '';
  }

  void _performCalculation() {
    if (operator != null) {
      switch (operator) {
        case Operator.Add:
          _textController.text = '${num1 + num2}';
          break;
        case Operator.Subtract:
          _textController.text = '${num1 - num2}';
          break;
        case Operator.Multiply:
          _textController.text = '${num1 * num2}';
          break;
        case Operator.Divide:
          _textController.text = '${num1 / num2}';
          break;
        case null:
          break;
      }
    }
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Calculator')),
      body: Padding(
        padding: const EdgeInsets.all(10),
        child: Column(
          children: [
            TextFormField(
              controller: _textController,
              textAlign: TextAlign.end,
              decoration: const InputDecoration(
                border: OutlineInputBorder(),
              ),
              style: const TextStyle(height: 0.5, fontSize: 30),
            ),
            const SizedBox(
              height: 5,
            ),
            Expanded(
              child: GridView.count(
                crossAxisCount: 4,
                mainAxisSpacing: 10,
                crossAxisSpacing: 10,
                childAspectRatio: 0.74,
                children: [
                  for (int i = 0; i < buttonText.length; i++) ...{
                    ElevatedButton(
                      onPressed: () {
                        _calculate(buttonText[i]);
                      },
                      child: Text(buttonText[i],
                          style: const TextStyle(fontSize: 30)),
                    )
                  }
                ],
              ),
            )
          ],
        ),
      ),
    );
  }
}
